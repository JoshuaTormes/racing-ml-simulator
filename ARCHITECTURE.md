...
1. `Car::observe()` monta o vetor de observação (10 floats)
...
**`Constants.h`** — todas as constantes da simulação como `constexpr`. Mudar `NUM_RAYS` aqui muda o tamanho do `Observation` em toda a cadeia automaticamente (via `OBS_SIZE = NUM_RAYS + 3`).

**`Types.h`** — os tipos de fronteira do ambiente de RL:

```cpp
using Observation = std::array<float, OBS_SIZE>; // 10 floats

struct Action      { float throttle, steering; }; // ambos ∈ [-1, 1]
struct StepResult  { Observation next; float reward; bool done; };
struct RewardConfig{ float w_progress, w_speed, w_idle, w_finish, w_crash, idle_eps; };
...
**Topologia padrão:** `{10, 8, 2}` — 10 entradas (OBS_SIZE), 8 ocultos, 2 saídas (throttle + steering).
...
│     ├── lê sensor.readings() [7 floats]                      │
│     ├── calcula speed_norm [1 float]                         │
│     ├── calcula angle_to_next_wp [1 float]                   │
│     └── calcula dist_to_next_wp [1 float]                    │
│         → Observation[10]                                    │
│                                                              │
...
│     ├── Sensor::update(pos, angle, track)  [7 raycasts]      │
...
### Observação — 10 floats

| Índice | Campo | Intervalo | Descrição |
|---|---|---|---|
| 0..6 | `ray_0` … `ray_6` | [0, 1] | Distância normalizada de cada raio ao obstáculo/borda mais próximo. 0 = borda imediata, 1 = nada até `RAY_MAX_LEN` |
| 7 | `speed_norm` | [0, 1] | `|speed| / MAX_SPEED` |
| 8 | `angle_to_next_wp` | [-1, 1] | Ângulo relativo ao próximo waypoint, dividido por π. 0 = direto à frente, ±1 = atrás |
| 9 | `dist_to_next_wp` | [0, 1] | Distância ao próximo waypoint, saturada em 400px |

Os raios são distribuídos em leque de 180° à frente:
```
ray_0 → aponta 90° à esquerda
ray_3 → aponta direto à frente
ray_6 → aponta 90° à direita
```
...
Por tick, para cada carro ativo:

```
r = w_progress × Δprogresso
  + w_speed    × (|speed| / MAX_SPEED)
  - w_idle     × (1 se |speed| < idle_eps)
  + w_finish   × (1 se done por Completed)
  - w_crash    × (1 se done por Collision)
```

**Δprogresso** é o incremento de `ProgressState.totalProg` no tick — proporcional a quantos waypoints foram avançados (0 se o carro ficou parado).

O **fitness** é a soma de `r` ao longo de todo o episódio. Na neuroevolução, é esse valor que o GA usa para seleção.

Defaults: `w_progress=1.0, w_speed=0.1, w_idle=0.05, w_finish=100, w_crash=50`.
...
`ProgressState` guarda `nextWp` (índice do próximo waypoint a alcançar) e `frac` (fração interpolada até ele).

A cada tick, se o carro está a menos de `0.75 * track_width` do waypoint `nextWp`, avança-se `nextWp++`. A fração é calculada pela projeção da posição no segmento `waypoints[nextWp-1] → waypoints[nextWp]`:

```
totalProg = (nextWp - 1) + frac
deltaProgress = totalProg - prevTotalProg
```

Isso produz um escalar monotonicamente crescente que funciona para pistas abertas e fechadas, e é diretamente usado no reward de progresso.
