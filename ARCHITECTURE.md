...
3. [Módulos](#módulos)
   ...
   - [sim/TrackGen — geração procedural](#simtrackgen--geração-procedural)
...
A regra mais importante da arquitetura: **`#include <SFML/...>` só pode aparecer em `render/Renderer.cpp`, `render/Renderer.h`, `control/HumanController.cpp` e `control/HumanController.h`**.
...
### `sim/TrackGen` — geração procedural
...
**`sim/TrackGen` (sem SFML)** — opera sobre `TrackData` (descrição de pista desacoplada de
...
1. Adicione um valor ao `enum class Obstacle::Type` em `sim/Track.h`
2. Carregue o JSON em `Track::Track()` em `sim/Track.cpp`
...
Reescreva apenas `render/Renderer.cpp/h` e `control/HumanController.cpp/h`. O restante do projeto não muda.
